...
import { handleHttpErrors } from "./errors/http-error-handler.js";

const createTvShowRoute = handleHttpErrors(
  Effect.gen(function* () {
...
const getTvShowRoute = handleHttpErrors(
  Effect.gen(function* () {
    const startedAt = Date.now();
...
const getTvShowsRoute = handleHttpErrors(
  Effect.gen(function* () {
    const startedAt = Date.now();
